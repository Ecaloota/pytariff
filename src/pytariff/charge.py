...
from old.typing import Consumption, Demand, MetricType
...
class TariffCharge(Generic[MetricType]):
    ...
    def validate_blocks_cannot_overlap(self) -> "TariffCharge":
        if any(
            [
                ...
                for j, y in enumerate(self.blocks)
                if i != j
            ]
        ):
            raise ValueError
    ...
    def __and__(
        self, other: "TariffCharge[MetricType]"
    ) -> "Optional[TariffCharge[MetricType]]":
...
class ConsumptionCharge(TariffCharge[Consumption]):
    ...
    def __and__(
        self, other: "TariffCharge[Consumption]"
    ) -> Optional["ConsumptionCharge"]:
        """"""
        # The intersection between two blocks defined in different units is empty
        if self.unit != other.unit:
            return None
