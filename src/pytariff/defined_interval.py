...
class DefinedInterval(BaseModel):
    ...
    def validate_model_is_aware(self) -> "DefinedInterval":
        if (
            helper.is_naive(self.start) or helper.is_naive(self.end)
    ...
    def validate_start_aware(self) -> "DefinedInterval":
        self.start = helper.convert_to_aware_datetime(
            obj=self.start, tzinfo=self.tzinfo
        )
        return self
    ...
    def __contains__(
        self,
        other: datetime | date | pd.Timestamp,
        tzinfo: Optional[timezone | ZoneInfo] = None,
    ) -> bool:
        """
