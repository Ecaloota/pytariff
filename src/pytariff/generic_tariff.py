...
from old.typing import MetricType
...
from pytariff.core.unit import TariffUnit, TradeDirection
...
class GenericTariff(DefinedInterval, Generic[MetricType]):
    ...
    def __contains__(
        self, other: datetime | date, tzinfo: Optional[timezone | ZoneInfo] = None
    ) -> bool:
    ...
    def apply_to(
        ...
        def _block_map_name(charge: TariffCharge) -> str:
            direction = (
                "_null_direction"
                if not charge.unit.direction
                else charge.unit.direction.value.lower()
            )
        ...
        def _cost_import_value(
            ...
            if (
                charge.unit.direction == TradeDirection.Import
                and block.rate
                and applied_map[idx]
            ):
                return (
                    block.rate.get_value(idx)
                    * getattr(charge_profile, _block_map_name(charge)).loc[idx]
                )
            return 0.0
        ...
        def _cost_export_value(
            ...
            if (
                charge.unit.direction == TradeDirection.Export
                and block.rate
                and applied_map[idx]
            ):
                return (
        ...
        resampled_meter = profile_handler._pytariff_resample(
            profile_handler.profile, child_resolution
        )
        ...
        for child in self.children:
            ...
            charge_profile = profile_handler._pytariff_transform(
                charge_profile, tariff_start, child.charge
            )
            ...
            for block in child.charge.blocks:
                ...
                block_map = getattr(charge_profile, _block_map_name(child.charge)).map(
                    block.__contains__
                )
                ...
                charge_profile[f"cost_import_{uuid_identifier}"] = cost_df[
                    "cost_import"
                ]
                charge_profile[f"cost_export_{uuid_identifier}"] = cost_df[
                    "cost_export"
                ]
            ...
            resampled_meter[f"cost_import_{child.uuid}"] = charge_profile.filter(
                like="cost_import"
            ).sum(axis=1)
            resampled_meter[f"cost_export_{child.uuid}"] = charge_profile.filter(
                like="cost_export"
            ).sum(axis=1)

        resampled_meter["import_cost"] = resampled_meter.filter(like="cost_import").sum(
            axis=1
        )
        resampled_meter["export_cost"] = resampled_meter.filter(like="cost_export").sum(
            axis=1
        )
        resampled_meter["total_cost"] = (
            resampled_meter["import_cost"] + resampled_meter["export_cost"]
        )
